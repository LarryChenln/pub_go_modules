...
func gatherInfoOutput(
	...
	for scanner.Scan() {
		...
		if !ok {
			...
			if "aa" == "aa" {
				kline := strings.TrimSpace(parts[1])
				gatherselfkeyvale(name, kline, acc, tags)
				continue
			}
...
//自定义key
var instantaneous_output_kbps_value, instantaneous_input_kbps_value int64 

func gatherselfkeyvale(
	name string,
	line string,
	acc telegraf.Accumulator,
	globalTags map[string]string,
	...
	for _, part := range parts {
		...
		case "instantaneous_input_kbps":
			ival, err := strconv.ParseInt(kv[1], 10, 64)
			if err == nil {
				instantaneous_input_kbps_value = ival
			}
		case "instantaneous_output_kbps":
			ival, err := strconv.ParseInt(kv[1], 10, 64)
			if err == nil {
				instantaneous_output_kbps_value = ival
			}
		case "instantaneous_input_and_output_kbps":
			_, err := strconv.ParseInt(kv[1], 10, 64)
			if err == nil {
				fields[kv[0]] = instantaneous_input_kbps_value + instantaneous_output_kbps_value
			}
		}
	}
	acc.AddFields("redis_selfkey", fields, tags)
}
